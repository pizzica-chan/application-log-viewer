...
同じ Web アプリケーションのセッションを追うための機能です。検索条件の下にある **セッション追跡** 欄を開いて使います（既定では閉じています）。
...
セッション ID を含む行を起点に、**同じログファイル・同じスレッド**で「はじまり」から「おわり」までを 1 リクエストとして、リクエストごとに見出しを付けて表示します。スレッドはプールで使い回されるため、スレッドだけでなく、はじまり・おわりで区切ります。はじまり・おわり・最大所要時間はブラウザに保存されます（セッション ID は保存しません）。
...
- 表示は最大 200 リクエスト、1 リクエストあたり最大 1,000 行です
- 追跡結果の「マッチ行をハイライト」は、スタックトレースの中にしか無い語には色が付きません（1 行目までの列だけで判定します。通常の一覧ではスタックトレースも対象です）
- 検索条件欄（レベル・期間など）の絞り込みは追跡には効きません。リクエスト内の行はレベルに関係なくすべて表示します（「検索」か「一覧に戻る」で、検索条件を適用した一覧に戻ります）
- `--fts` なしでは起点探しが全件走査になります（一覧の全文検索と同じ）。実測値は `SessionTrace` のコメントを参照

**サンプル**: `samples/session/` に試験用のログがあります。`--dir samples` で起動し、上の表の例をそのまま入力すると、次の 7 リクエストが表示されます。
