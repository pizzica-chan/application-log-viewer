...
`--fts` の FTS5 索引は本文の複製も語の位置も持たない構成で、正規表現メタ文字を含まない
3 文字以上（コードポイントで数えるので、絵文字もそれぞれ 1 文字）の grep でだけ候補の絞り込みに
使います。最終的な一致は生ログで判定するので、結果は `--fts` なしと同じです
（詳しくはリポジトリ直下の [README.md](../README.md#機能) の全文検索の段落）。
