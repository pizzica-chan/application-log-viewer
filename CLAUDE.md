...
- 取り込むログの書式は `LogFormat` で切り替える。**取り込み開始時に 1 つへ確定させる**ため、
  書式を増やしても 1 行あたりの判定は 1 書式ぶんで済む。この前提を崩さないこと
  （行ごとに複数書式を試すと継続行の扱いが重くなる）。実測は
  [docs/performance-report.md](docs/performance-report.md) の 9 章
